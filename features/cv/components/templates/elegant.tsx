...
export function TemplateElegant({ informations, sections, couleurAccent, police }: ProprietesTemplate) {
    ...
    >
      <header className="relative pt-8 pb-10 px-10 text-white" style={{ backgroundColor: couleurAccent }}>
        ...
        {informations.titrePoste && (
          <p className="text-sm mt-1.5 text-white/85 tracking-wide">{informations.titrePoste}</p>
        )}

        <div className="flex gap-4 mt-4 text-[11px] text-white/80 flex-wrap">
          {informations.email && (
      ...
      <div className={informations.photoUrl ? "px-10 pt-4" : "px-10 pt-10"}>
        {informations.resume && (
          <p className="text-sm leading-relaxed mb-8 text-[#3D4B5C] max-w-lg">{informations.resume}</p>
        )}
          ...
          {sections
            ...
            .map((section) => (
              <section key={section.id}>
                <h2
                  className="text-sm font-medium mb-3.5 flex items-center gap-2"
